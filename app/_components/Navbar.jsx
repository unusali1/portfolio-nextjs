...
import logoUnus from "../../public/assets/logoWhite.png";
import logoUnusBlack from "../../public/assets/logoBlack.png";
...
export default function Navbar() {
  ...
  return (
    <header
    className={`z-50 transition-all duration-300 a
        ${headerActive
          ? "fixed top-0 left-0 right-0 shadow-md bg-white dark:bg-[#212A4A] animate-slideIn"
          : "absolute top-0 sm:top-10 sm:left-1/2 sm:-translate-x-1/2 bg-gray-700 rounded-md sm:w-[90%] w-full"
        }
        ...
        <div className="hidden md:flex flex-1 justify-center items-center">
          <ul className="flex gap-8 text-lg font-medium">
            {navigationData.map((item, index) => (
              <li key={index} className="font-salsa">
                <Link
                  href={item.link}
                  className={`${
                    pathname === item.link
                      ? "text-orange-500 border-b-2 border-orange-500"
                      : headerActive ? "text-gray-800  dark:text-gray-100" :" text-gray-100"
        ...
        <ul className="hidden md:flex gap-4 items-center">
          {socialMediaLinks.map((item,index) => (
            <li key={index}>
              <a
                ...
                className={`
                  ${headerActive
                    ? "text-xl text-gray-800 hover:text-pink-500 transition-colors  dark:text-gray-100"
                    : "text-xl text-white hover:text-pink-500 transition-colors"
                  }
                  `}
              >
         ...
         <div className="ml-3">
              <button
                ...
                className="p-2 bg-gray-200 dark:bg-gray-600 px-2 py-2 rounded-full mt-1"
              >
                {isDarkMode ? (
                  <Icon
